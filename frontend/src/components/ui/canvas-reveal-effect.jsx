...
export const CanvasRevealEffect = ({
  animationSpeed = 1.5,
  opacities = [0.3, 0.4, 0.5, 0.6, 0.7, 0.8, 0.9, 1.0, 0.8, 0.9],
  colors = [[255, 77, 109], [255, 133, 161], [255, 92, 138]],
  containerClassName,
  dotSize = 3,
  showGradient = false,
}) => {
      ...
      <div className="h-full w-full">
        <DotMatrix
          ...
          shader={`
            ...
            float intro_offset = distance(u_resolution / 2.0 / u_total_size, st2) * 0.005 + (random(st2) * 0.05);
            opacity *= step(intro_offset, u_time * animation_speed_factor);
            opacity *= clamp((1.0 - step(intro_offset + 0.05, u_time * animation_speed_factor)) * 2.0, 1.0, 2.0);
          `}
          center={["x", "y"]}
        />
      </div>
...
// ShaderMaterial Logic with FPS limiting
    ...
    for (const key in uniforms) {
      ...
      switch (type) {
        ...
        case "uniform1fv":
          ...
          break;
        case "uniform2f":
          prepared[key] = { value: new THREE.Vector2().fromArray(value) };
          break;
        case "uniform3f":
          prepared[key] = { value: new THREE.Vector3().fromArray(value) };
          break;
        case "uniform3fv":
          prepared[key] = { value: value.map((v) => new THREE.Vector3().fromArray(v)) };
          break;
        default:
